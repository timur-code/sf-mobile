...
//  RANT
//
//  Created by Aligazy Kismetov on 22.04.2022.
...
class WithdrawViewController: UIViewController {
    ...
    @IBAction func withdraw(_ sender: Any) {
        ...
        let myUrl = URL(string: "https://sf-rant-backend.herokuapp.com/user/\(id!)/withdraw")
                var request = URLRequest(url:myUrl!)
                request.httpMethod = "POST"// Compose a query string
                request.addValue("application/json", forHTTPHeaderField: "content-type")
                request.addValue("application/json", forHTTPHeaderField: "Accept")
                request.addValue("Bearer \(accessToken!)", forHTTPHeaderField: "Authorization")
        print("amount = \(field.text!)")
                let postString = ["amount": field.text!
                                  ] as [String: String]
        do {
     ...
     let task = URLSession.shared.dataTask(with: request) { (data: Data?, response: URLResponse?, error: Error?) in
        ...
        if error != nil
        {
         ...
         self.displayMessage(userMessage: "Withdraw done")
        }
